...
<script>
let sessionId=null; let debounce=null; let liveImages={he:null,oct:null,mask:null}; let batchId=null; let batchTimer=null; let maskOverlayRaf={};
let editors={};
...
function opacityNumberAdjust(){ const v=Math.max(0,Math.min(1,parseFloat(heOpacityV.value)||0)); heOpacityV.value=v; heOpacity.value=v; drawLiveOverlay(); }
...
function scheduleMaskOverlayPreview(id){ if(maskOverlayRaf[id])return; maskOverlayRaf[id]=requestAnimationFrame(()=>{ maskOverlayRaf[id]=null; const c=document.getElementById(id); const target=id==='octCanvas'?octMaskOverlay:(id==='heCanvas'?heMaskOverlay:null); if(c&&target)target.src=c.toDataURL('image/png'); }); }
function redrawEditor(id){ const c=document.getElementById(id); const ctx=c.getContext('2d'); const ed=editors[id]; if(!ed)return; ctx.clearRect(0,0,c.width,c.height); ctx.drawImage(ed.base,0,0,c.width,c.height); const maskData=ed.maskCtx.getImageData(0,0,c.width,c.height); const overlay=ctx.getImageData(0,0,c.width,c.height); for(let i=0;i<maskData.data.length;i+=4){ if(maskData.data[i]>127){ overlay.data[i]=Math.round(overlay.data[i]*0.55+ed.color[0]*0.45); overlay.data[i+1]=Math.round(overlay.data[i+1]*0.55+ed.color[1]*0.45); overlay.data[i+2]=Math.round(overlay.data[i+2]*0.55+ed.color[2]*0.45); }} ctx.putImageData(overlay,0,0); scheduleMaskOverlayPreview(id); }
...
function manualAdjust(){ updateManualDisplays(); drawLiveOverlay(); clearTimeout(debounce); debounce=setTimeout(async()=>{ setStatus('Applying manual adjustment to native outputs...'); await api('/api/manual',{session_id:sessionId,scale:parseFloat(mScale.value),stretch_x:parseFloat(mStretchX.value),stretch_y:parseFloat(mStretchY.value),rotation_deg:parseFloat(mRot.value),translation_y:parseFloat(mTy.value),translation_x:parseFloat(mTx.value)}); await refreshReg(); setStatus('Manual adjustment applied.'); },650); }
