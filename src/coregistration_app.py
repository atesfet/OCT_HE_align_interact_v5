...
async function saveFinal(){ await withBusy('busy-save','Saving final outputs...', async()=>{ await api('/api/manual',manualPayload()); await api('/api/apply_manual',{session_id:sessionId}); const j=await api('/api/save',{session_id:sessionId}); await refreshReg(); saveLinks.innerHTML = j.files.map(f=>`<div><a href="/api/file?session=${sessionId}&name=${f}" target="_blank">${f}</a></div>`).join(''); setStatus(JSON.stringify(j,null,2)); }); }
</script>
</body>
</html>
"""
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        try:
            ...
            elif parsed.path == "/api/apply_manual":
                payload = _read_json(self)
                paths = _session(payload["session_id"])
                state = json.loads((paths.root / "transform_state.json").read_text())
                _apply_current_transform(paths, state)
                _json_response(self, {"ok": True, "manual": state.get("manual", {})})
